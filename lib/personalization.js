...
function buildVars(booking, extras = {}) {
  ...
  // Weather vars
  let weatherTemp = extras.weather ? `${extras.weather.temp}°C` : '24°C';
  let weatherCondition = extras.weather ? extras.weather.condition : 'Pleasant';
  let weatherNote = extras.weather
    ? `Current conditions: ${weatherCondition}, ${extras.weather.temp}°C.`
    : '';
  ...
  if (extras.currentDay) {
    dayLabel = extras.currentDay === 1
      ? `Check-in · Day 1`
      : `Day ${extras.currentDay} of ${nights}`;
  }
  ...
  return {
    ...
    ReviewLink: process.env.REVIEW_LINK || 'https://g.page/inika-resorts/review',
  };
}
